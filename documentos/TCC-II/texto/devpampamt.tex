\chapter{DESENVOLVIMENTO E RESULTADOS DO {PampaMT}}
    ...
    Após a seleção, os dados  são copiados para o diretório: \textbf{DADOS\_MT/projeto}. Esse processo é realizado para prever eventuais perdas dos arquivos. Os dados então são convertidos e salvos no diretório: \textbf{PROC\_MT/projeto} 
    
    O usuário será levado a tela do processamento EMTF. O processo já estabelece algumas configurações padrão, entretanto o usuário pode alterar qualquer configuração, tais como, escolher uma nova janela ou alterar o horário do relógio dos dados (Figura \ref{conf-procZ}).
    ...
    Após escolher os melhores períodos o usuário pode executar o \en{script}: \textbf{ToJones}, onde esse é realizado ao pressionar o botão no canto inferior esquerdo. O PampaMT abre uma caixa de diálogo para nomear o arquivo de saída, e executa o \en{script}, finalizando a ultima etapa do pré-processamento.
